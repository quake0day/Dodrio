...
if __name__ == "__main__":
    ...
    if len(sys.argv) != 2:
        ...
        if sys.argv[1].strip() == "help":
            ...
            exit()
    scrape_publication(url_to_scrape)
...
def scrape_paper(url_to_scrape):
    ...
    for div in divs:
        ...
        if str(div).startswith('<div style="margin-bottom:1em">'):
            citations_data = (div.get_text(strip=True))
            m = re.search('(?<=by\ )\w+', citations_data)
            citations = m.group(0)
        count+=1
